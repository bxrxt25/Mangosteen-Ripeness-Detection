...
// ปิด face detect/recognition ของ esp-dl ทั้งคู่ เพื่อคืน flash และ PSRAM
// ให้กับโมเดล mangosteen_int8.h (TFLite Micro) แทน
...
static uint32_t ai_color_for_fb(uint32_t color)
{
    // fb_gfx ใช้ 0x00BBGGRR กับ RGB565 frame buffer
    return ((color >> 16) & 0x001F) | ((color >> 3) & 0x07E0) |
           ((color << 8) & 0xF800);
}

static void draw_ai_overlay(camera_fb_t *fb)
{
    if (!fb || fb->format != PIXFORMAT_RGB565) return;

    MangosteenResult result = {};
    const bool has_result = mangosteen_get_latest_result(&result);
    const uint32_t age_ms = has_result ? millis() - result.updated_ms : 0;

    char text[80];
    uint32_t foreground = 0x00FFFFFF; // white
    if (!has_result) {
        snprintf(text, sizeof(text), "AI: starting...");
    } else if (age_ms > 2000) {
        snprintf(text, sizeof(text), "AI: waiting for frame");
        foreground = 0x0000FFFF; // yellow
    } else {
        snprintf(text, sizeof(text), "AI: %s  %.0f%%  %lums",
                 mangosteen_class_label(result.class_index),
                 result.confidence * 100.0f,
                 (unsigned long)result.inference_ms);
        if (result.class_index == 0) foreground = 0x000000FF;      // red
        else if (result.class_index == 1) foreground = 0x0000FFFF; // yellow
        else foreground = 0x0000FF00;                               // green
    }
    ...
    rfb.bytes_per_pixel = 2;
    rfb.format = FB_RGB565;
    const int box_width = min((int)fb->width, 250);
    fb_gfx_fillRect(&rfb, 0, 0, box_width, 24, ai_color_for_fb(0x00000000));
    fb_gfx_print(&rfb, 8, 7, ai_color_for_fb(foreground), text);
}
...
static esp_err_t ai_status_handler(httpd_req_t *req)
{
    ...
    if (!has_result) {
        snprintf(json, sizeof(json),
                 "{\"ready\":false,\"message\":\"waiting for first inference\"}");
    } else {
        const uint32_t age_ms = millis() - result.updated_ms;
        snprintf(json, sizeof(json),
                 "{\"ready\":true,\"class_index\":%d,\"label\":\"%s\","
                 "\"confidence\":%.4f,\"scores\":[%.4f,%.4f,%.4f],"
                 "\"inference_ms\":%lu,\"age_ms\":%lu}",
                 result.class_index, mangosteen_class_label(result.class_index),
                 result.confidence, result.scores[0], result.scores[1], result.scores[2],
                 (unsigned long)result.inference_ms, (unsigned long)age_ms);
    }
...
void startCameraServer()
{
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(camera_httpd, &ai_status_uri);
